...
my $smartctl = which( "smartctl", $PATH );
my $mdadm = which( "mdadm", $PATH );
my $megacli= which( "megacli", $PATH );
my $arcconf = which( "arcconf", $PATH );
...
sub check_disk_utilities {
    ...
    if ($adaptec_needed) {
        die "Adaptec utility not found. Please, install Adaptech raid management utility into " . $arcconf . "\n" unless -e $arcconf;
    }

    if ($lsi_needed) {
        die "Megacli not found. Please, install LSI MegaCli raid management utility into " . $megacli . " (symlink if needed)\n" unless -e $megacli
    }
